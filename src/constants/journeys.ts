...
export interface Journey {
  ...
  title: string;
  subtitle: string;
  /** Localized journey titles keyed by language code (en, ka, …). */
  title_i18n?: Partial<Record<string, string>>;
  /** Localized journey subtitles keyed by language code (en, ka, …). */
  subtitle_i18n?: Partial<Record<string, string>>;
  icon: keyof typeof Ionicons.glyphMap;
  accent: string;
  steps: JourneyStep[];
}

/** Localized journey title — falls back to English `title`. */
export function getJourneyTitle(journey: Journey, lang: string): string {
  return journey.title_i18n?.[lang] || journey.title_i18n?.en || journey.title;
}

/** Localized journey subtitle — falls back to English `subtitle`. */
export function getJourneySubtitle(journey: Journey, lang: string): string {
  return journey.subtitle_i18n?.[lang] || journey.subtitle_i18n?.en || journey.subtitle;
}
